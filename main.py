...
- Stores data in a Supabase (hosted Postgres) database
...
Database: Supabase
-------------------
Supabase is a managed Postgres database, so this backend talks to it the
same way it would talk to any Postgres instance: over SQLAlchemy with the
psycopg2 driver, using the connection string from your Supabase project's
Settings -> Database page. No SQLite fallback is used.

Recommended connection string to use for DATABASE_URL:
- App runs long-lived (this service, a VM, Docker, Render, etc.) ->
  use the "Session pooler" (or "Direct connection") string, port 5432/6543.
- App is serverless / very short-lived connections (e.g. edge functions) ->
  use the "Transaction pooler" string (port 6543). This backend detects
  pgbouncer/transaction-pooler URLs automatically and disables SQLAlchemy's
  own connection pooling to avoid conflicting with it.
...
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Boolean, desc, func, Text, inspect, text
...
try:
    # Optional convenience for local development: if a .env file is present
    # (e.g. copied from .env.example) load it. Never required in
    # production, where the host (Render, Docker, etc.) injects real env
    # vars directly.
...
if not DATABASE_URL:
    raise RuntimeError(
        "DATABASE_URL is not set. This backend requires a Supabase "
        "Postgres connection string, e.g.\n"
        "  postgresql://postgres.xxxx:[PASSWORD]@aws-0-<region>.pooler.supabase.com:5432/postgres\n"
        "Find it under your Supabase project -> Settings -> Database -> "
        "Connection string, and set it as the DATABASE_URL environment "
        "variable (see .env.example)."
    )

# SQLAlchemy/psycopg2 expect the "postgresql://" scheme; Supabase (and many
# other providers) sometimes hand out the older "postgres://" form.
...
# Supabase's "Transaction pooler" (Supavisor/pgbouncer, typically port 6543
# or a host containing "pooler.supabase.com" with pgbouncer semantics)
# already pools connections for you and doesn't support some session-level
# features SQLAlchemy's own pool relies on. When it looks like that's what
# we're talking to, disable SQLAlchemy-side pooling (NullPool) so every
# request just borrows/returns a pooler connection directly.
_is_transaction_pooler = (
    "pooler.supabase.com" in (_parsed_url.host or "") and _parsed_url.port == 6543
) or "pgbouncer=true" in DATABASE_URL.lower()

_engine_kwargs: Dict[str, Any] = {
    # Require SSL when the caller hasn't already specified an sslmode -
    # Supabase requires an encrypted connection.
    "connect_args": {"sslmode": "require"} if "sslmode=" not in DATABASE_URL.lower() else {},
    # Recycle/pre-ping so long-idle connections (very common with a
    # hosted DB reached over the internet) don't surface as crashes.
    "pool_pre_ping": True,
    "pool_recycle": 300,
}
if _is_transaction_pooler:
    _engine_kwargs["poolclass"] = NullPool
    del _engine_kwargs["pool_recycle"]  # not meaningful with NullPool
...
class GPSData(Base):
    ...
    # (e.g. right after reboot, before a fresh satellite lock) rather than
    # a live GPS reading.
    stale = Column(Boolean, default=False, nullable=False)
    # True when this point was sent late from the device's offline SPIFFS
    # buffer rather than in real time.
    buffered = Column(Boolean, default=False, nullable=False)
...
def run_lightweight_migrations():
    """
    Add newly-introduced columns to an already-existing gps_data table.

    Base.metadata.create_all() only creates tables that don't exist yet —
    it never alters a table that's already there. So on a fresh database
    the new stale/buffered columns are created automatically, but on an
    already-deployed database (e.g. a Supabase project created before
    this change) they need to be added once by hand. This does that,
    safely and idempotently.
    """
    inspector = inspect(engine)
    if "gps_data" not in inspector.get_table_names():
        return  # brand-new DB; create_all() already made the table correctly

    existing_cols = {c["name"] for c in inspector.get_columns("gps_data")}
    bool_default = "false"

    with engine.begin() as conn:
        if "stale" not in existing_cols:
            conn.execute(text(
                f"ALTER TABLE gps_data ADD COLUMN stale BOOLEAN NOT NULL DEFAULT {bool_default}"
            ))
            print("[Migration] Added gps_data.stale column")
        if "buffered" not in existing_cols:
            conn.execute(text(
                f"ALTER TABLE gps_data ADD COLUMN buffered BOOLEAN NOT NULL DEFAULT {bool_default}"
            ))
            print("[Migration] Added gps_data.buffered column")
...
class GPSDataCreate(BaseModel):
    ...
    # Sent by the ESP32 firmware; both optional so older firmware that
...
class DeviceUpdate(BaseModel):
    """Body for PATCH /api/devices/{device_id}. Both fields optional — only
    the ones present are applied. (Sent as a JSON body by the frontend, not
    query params.)"""
...
# =============================================================================
def _device_to_response(db: Session, device: Device) -> DeviceResponse:
    """Build a DeviceResponse including record_count (not a DB column —
    computed from gps_data on the fly)."""
    count = db.query(GPSData).filter(GPSData.device_id == device.device_id).count()
    item = DeviceResponse.model_validate(device)
...
async def get_devices(
    ...
    """Get all registered devices, including each device's GPS record count."""
    devices = db.query(Device).order_by(Device.last_seen.desc()).all()

    counts = dict(
        db.query(GPSData.device_id, func.count(GPSData.id))
        .group_by(GPSData.device_id)
        .all()
    )
    result = []
    for d in devices:
        item = DeviceResponse.model_validate(d)
        item.record_count = counts.get(d.device_id, 0)
        result.append(item)
    return result
...
async def update_device(
    ...
    Takes a JSON body (DeviceUpdate) — matches the frontend, which PATCHes
    with `{ name }` / `{ is_active }` as a JSON payload, not query params.
    """
    device = db.query(Device).filter(Device.device_id == device_id).first()
    if not device:
        raise HTTPException(status_code=404, detail="Device not found")
...
async def health_check():
    """Health check endpoint. Also verifies the Supabase DB is reachable,
    since that's the most common failure mode for this service (wrong
    connection string, project paused, network egress blocked, etc.)."""
    db_status = "ok"
    try:
